...
let EnderecosService = class EnderecosService {
    ...
    async createForUser(dto, userId) {
    ...
    async createForLoja(dto, vendedorId) {
    ...
    async update(enderecoId, userId, dto) {
        ...
        if (!endereco) {
            throw new common_1.ForbiddenException('O endereço não encontrado.');
        }
        this.repository.merge(endereco, dto);
        return this.repository.save(endereco);
    }
    async updateLoja(userId, dto) {
        const endereco = await this.repository.findOne({
            where: {
    ...
    async remove(enderecoId, userId) {
        const endereco = await this.repository.findOne({
            where: {
                id: enderecoId,
                cliente: { id: userId }
